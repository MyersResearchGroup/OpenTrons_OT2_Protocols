...
metadata = {
    'protocolName': 'multi-channel consolidate',
    'author': 'Parke',
    'description': 'Transfer from multiple columns to one column on other plate',
    'apiLevel': '2.10'
}
...
def run(protocol):
    ...
    for src, d in zip([source],dest): 
        for s in src:
            pipette.consolidate(volume, plate_1[s], plate_2[d])
